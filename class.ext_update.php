...
class ext_update
{
    ...
    public function main()
    {
        if (!GeneralUtility::_GP('do_initialize')) {
            ...
            $onClick = "document.location='"
                . GeneralUtility::linkThisScript(array('do_initialize'=>1))
                . "'; return false;"
            ;
            $content .= htmlspecialchars($GLOBALS['LANG']->getLL('update_convert_now')).'
                <br /><br />
                <form action=""><input type="submit" value="'
                . LocalizationUtility::translate('doInitialize', 'ameos_filemanager')
                . '" onclick="' . htmlspecialchars($onClick) . '"></form>'
            ;
        } else {
            $this->countFolder = 0;
            ...
            $updated = $this->update();
            $content .= $updated . '<br/>'
                . LocalizationUtility::translate('initializeCountFolder', 'ameos_filemanager')
                . ' : ' . $this->countFolder . ' - '
                . $this->countAddedFolder . '<br/>'
                . LocalizationUtility::translate('adviceInitialize', 'ameos_filemanager')
            ;
            ...
            <div style="padding:15px 15px 20px 0;">
                <div class="typo3-message message-warning">
                    <div class="message-header">'
                        . LocalizationUtility::translate('warning', 'ameos_filemanager')
                    . '</div>
                    <div class="message-body">'
                        . LocalizationUtility::translate('warningInitialize', 'ameos_filemanager')
                    . '</div>
                </div>
            </div>'
        ;
                ...
                if ($entry != "." && $entry != "..") {
                    if (is_dir($folder . $entry)) {
                        $res = $GLOBALS['TYPO3_DB']->exec_SELECTquery(
                            ...
                            "tx_ameosfilemanager_domain_model_folder.title like '". $entry ."'"
                        );
                        $exist = false;
                        while ($row = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($res)) {
                            if (Tools::getFolderPathFromUid($row['uid'])
                                == str_replace($storageFolderPath, '', $folder . $entry)
                            ) {
                                $exist = true;
                                $uid = $row['uid'];
                                break;
    ...
    protected function setDatabaseForFile($storageFolderPath, $folderPath, $entry, $folderParentUid)
    {
        ...
        $file = $GLOBALS['TYPO3_DB']->exec_SELECTgetSingleRow(
            'uid',
            'sys_file',
            "identifier LIKE '" . $fileIdentifier . "'"
        );
        if ($file) {
            $fileUid = $file['uid'];
